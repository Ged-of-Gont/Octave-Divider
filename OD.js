  ...
  function toggleMidiControls() {
    if (formatSelect.value === "json") {
      ...
      midiControls.style.display = "flex";  // or "block" if that fits your layout better
    }
  }
  ...
  function animate(){
    ...
    // 1) update & draw the fuzzy particles
    updateParticles();
    drawParticles();

    // 2) render the scale lines & labels
    renderScale();
  }
  ...
  function createParticlesGaussian(deg, count = DEFAULT_PARTICLE_COUNT, laplaceScale = 10) {
    ...
    // The x-coordinate of the vertical tone marker
    const centerX = ratioToX(deg.floatVal, LEFT_X, WIDTH);
    const centerY = MID_Y;

    // For the x offset, we use Laplace as in your original.
    const sigmaY = 30;

    // Compute hue based on ratio
  ...
  function renderScale(){
    ...
    // Draw interval labels between consecutive degrees
    ...
    // For each scale degree, render marker, glow, and labels.
    for(let deg of scaleDegrees) {
      ...
      // Restore original glowing behavior using hue computed from ratio.
